...
// All 30 NBA team abbreviations grouped by conference.
const TEAMS = {
  East: [
...
export function GamePicker() {
  ...
  // Load all games for the season once. We filter client-side by team.
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setError(null);
  ...
  // Reset to team-selection stage whenever season changes.
  useEffect(() => {
    setStage("team");
  ...
  return (
    <div className="border-r border-[#1F2230] bg-[#0F1117] w-72 flex flex-col h-full">
      ...
      <div className="overflow-y-auto flex-1">
        {loading && (
          <div className="px-6 py-4 font-mono text-xs text-text-secondary">loading…</div>
        )}
        {error && (
          <div className="px-6 py-4 font-mono text-xs text-prob-loss">{error}</div>
        )}
          ...
          <div>
            <div className="px-6 py-3 bg-[#0B0D12] border-b border-[#1F2230]">
              ...
              <div className="font-mono text-xs text-text-secondary">
                {teamGames.length} games · {season}
              </div>
            </div>
            {teamGames.length === 0 ? (
              <div className="px-6 py-4 font-mono text-xs text-text-secondary">
                no games found for this team in this season
              </div>
            ) : (
              teamGames.map((g) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-6 py-3 border-b border-[#1F2230] hover:bg-[#15171F] transition-colors ${
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-mono text-xs tabular-nums">
                        <span className={won ? "text-prob-win" : "text-prob-loss"}>
                          {won ? "W" : "L"}
                        </span>
